...
module.exports = function migrator(elNode, context) {
    ...
    if (!attributes.length || attributes.some(attr => attr.name === "name" && attr.value.type === "Literal")) {
        return;
    }
    ...
    params.slice().reverse().forEach(param => {
        ...
        const value = builder.memberExpression(builder.identifier("macroInput"), name);
        elNode.prependChild(builder.scriptlet({
            value: printJS(builder.vars([builder.variableDeclarator(name, value)]), context)
        }));
    });
    ...
    const walker = context.createWalker({
        enter(child) {
            if (child.type === "HtmlElement" && child.tagName === name && child.argument) {
