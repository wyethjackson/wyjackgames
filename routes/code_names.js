...
router.get('/:code_name_id', async function (req, res) {
  ...
  res.marko(index, {path: '../', page_id: 'PROJECTS', active_page: 'code_names', code_names: code_name_data, hide_nav: true, host_url: process.env.HOST_URL});
});



router.get('/:code_name_id/ajax', async function (req, res) {
  const code_name_data = await getGameDetails(req.params.code_name_id);
  res.json(code_name_data);
});

router.post('/:code_name_id/ajax', async function (req, res) {
...
router.post('/new', async function (req, res) {
  ...
  res.redirect(`/code_names/${code_name_id}`);
});
